...
#### **Exercise**: Create a subset of the Canadian salary data, `sal_ca`, that shows only entry level employees from mid-size companies and only the `experience_level`, `job_title`, `company_size`, and `salary_in_usd` columns.
...
```r
sal_ca[
        sal_ca$experience_level == 'EN'                                     #<1> 
        & sal_ca$company_size == 'M'                                        #<2>
        , 
        c('experience_level', 'job_title', 'company_size', 'salary_in_usd') #<3>
        ]
```
1. Filter on `experience_level` equal to entry level or `EN` in the data.
2. AND (specify with `&`) filter on `company_size` equal to mid-size or `M` in the data.
3. Select columns to the right of the `,` in a vector `c()`.
...
```r
sal[
        is.na(sal$salary_in_usd)                         #<1> 
        & sal$work_year == 2020                          #<2>
        ,                                                #<3>
        ]
```
1. Filter where `salary_in_usd` is blank.
2. AND (specify with `&`) filter on `work_year` equal to `2020`. Notice that `2020` does not need to be in quotes because it is a numeric value. Note the `==` instead of `=` when using a logical statement as a filtering criteria.
3. Select all columns by leaving blank after the `,`.
